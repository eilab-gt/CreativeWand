...
from CreativeWand.Application.Deployment.PresetObjects import create_session
from CreativeWand.Application.Instances.Frontend.WebFrontendHelper.WebFrontendSessionManager import SessionManager
...
class WebFrontendServer():
    ...
    def start_server(self, run_async=True):
        """
        ...
        if run_async:
            t = threading.Thread(
                target=lambda: self.socketio.run(self.app, host='0.0.0.0', port=8000, debug=False,
                                                 use_reloader=False))
            t.start()
        else:
    ...
    def send_chat_message(self, message, session_id, wait):
        """
        ...
        """
        time.sleep(0.3)
        self.socketio.emit('message', {"message": message, "id": session_id}, json=True)
        if wait:
    ...
    def home(self):
        ...
        creative_session = create_session(
            ...
            em_info={"session_id": this_id, "session_type": presets}
        )
